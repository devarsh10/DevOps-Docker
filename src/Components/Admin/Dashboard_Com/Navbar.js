...
export default function NavBar() {
  ...
  const handleSaveChange = async (e) => {
    ...
    const id = sessionStorage.getItem("id");
    await axios
    .patch(`http://localhost:8000/admin/${id}`, {
  ...
  const renderMenu = (
    ...
    >
      ...
      <MenuItem onClick={handleMenuClose}><Link to='/login' style={{color:"black",textDecoration:"None"}}>Log Out</Link></MenuItem>
    </Menu>
  );
    ...
    <Box sx={{ flexGrow: 1 }}>
      ...
      >
        <Toolbar>
          ...
          >
          <Link to="/dashboard" style={{color:"white",textDecoration:"None"}}>Admin Portal</Link>
          </Typography>
          <Search>
            <SearchIconWrapper>
              <SearchIcon />
