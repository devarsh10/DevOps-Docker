...
export default function RejectedReferralList() {
  // State variables
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(5);
  const [rows, setRows] = useState([]);
  const agentId = sessionStorage.getItem("id"); // Access agentId from URL parameter
  ...
  const fetchReferrals = async (agentId) => {
    try {
      ...
      const rejectedReferrals = referralData.filter(
        (referral) => referral.status === "Rejected"
      );
      setRows(rejectedReferrals);
    } catch (error) {
          ...
          <Container>
            <Paper className="table-container">
              <Typography className="table-heading" variant="h5">
                Referrals with status Rejected
              </Typography>
              <Divider />
              <TableContainer>
